...
if [ ! -f "$HOME/.profile_scripts/aliases.sh" ];then
	...
	echo 'alias sshon="systemctl start ssh"' > $HOME/.profile_scripts/aliases.sh
	echo 'alias sshoff="systemctl stop ssh"' >> $HOME/.profile_scripts/aliases.sh
fi

# Now to replace the pocket-home configuration file with our templated one and to
# replace the Help icon with a web browser.
echo '*****************************************'
echo 'We can switch out the help file icon'
...
echo 'have made to the home screen.'
echo '*****************************************'
echo -en 'Replace Help with Browser? (y/n)>';read answer
